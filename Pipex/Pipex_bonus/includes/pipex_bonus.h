/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   pipex_bonus.h                                      :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: tpotilli <tpotilli@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/07/10 16:39:00 by tpotilli          #+#    #+#             */
/*   Updated: 2023/10/18 17:32:52 by tpotilli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#ifndef PIPEX_BONUS_H
# define PIPEX_BONUS_H

// =======================================================================
//								Colors
// =======================================================================

# define BLACK "\001\033[1;30m\002"
# define RED "\001\033[1;31m\002"
# define GREEN "\001\033[1;32m\002"
# define YELLOW "\001\033[1;33m\002"
# define BLUE "\001\033[1;34m\002"
# define PURPLE "\001\033[1;35m\002"
# define CYAN "\001\033[1;36m\002"
# define WHITE "\001\033[1;37m\002"
# define RESET "\001\033[0m\002"

// =======================================================================
//								Include
// =======================================================================

# include <fcntl.h>
# include <stdlib.h>
# include <stdio.h>
# include <string.h>
# include <sys/types.h>
# include <sys/wait.h>
# include <unistd.h>
# include <signal.h>

// =======================================================================
//								Structure
// =======================================================================

typedef struct s_struct
{
	int		argc;
	struct	s_struct *next;
	char	**argv;
	char	**envp;

}		t_struct;

t_struct	*ft_createcell(char **argv, char **env, int argc);
t_struct	*ft_init_struct(t_struct *data);
void		free_all_struct(t_struct *list);
void		ft_free_list(t_struct *list2);
void		free_env(t_struct *list);

// =======================================================================
//								Libft
// =======================================================================

int			ft_letter(char const *s, char c, int i);
char		**ft_free(char **split, int index_word);
int			ft_word(char const *s, char c);
char		**ft_copy(char const *s, char c, char **split, int i);
char		**ft_split(char const *s, char c);
size_t		ft_strlen(const char *s);
int			ft_strncmp(const char *s1, const char *s2, size_t n);
char		*ft_strnstr(const char *big, const char *little, size_t len);
int			ft_check(char const *s, size_t len, unsigned int start);
char		*ft_substr(char const *s, unsigned int start, size_t len);
char		*ft_strdup(const char *s);
char		*ft_cpy(char const *s1, char const *s2, char *str);
char		*ft_strjoin(char *s1, char *s2);

// =======================================================================
//								Pipex
// =======================================================================

int			ft_parsing_bonus(int argc);
int			ft_verif_error_bonus(char *buff, int fd[]);
int			ft_pipex_bonus(int fd1, int fd2, t_struct *list);
int			ft_create_fd_bonus(char *argv, int flag);
void		child_process_bonus(int fd1, t_struct *list, int *end);
void		parent_process_bonus(int fd4, t_struct *list, int *end);
char		*ft_get_pass_bonus(char *argv, char *envp, int i);
void		ft_do_process_bonus(t_struct *list, char *cmd);
char		**ft_take_line_bonus(char **big, char *little);
void		ft_freedb_bonus(char **str);
void		ft_execute_bonus(char	*cmd, char	**env);
char		*ft_strjoin_free_bonus(char *s1, char *s2);
char		**ft_get_path_bonus(char **env);
char		*ft_strchr_bonus(const char *string, int searchedChar);
int			not_Path_bonus(const char *s1, const char *s2);
char		*str_join_free_bonus(char *path, char *cmd);
int			count_len_db_bonus(char *argv[]);

#endif
